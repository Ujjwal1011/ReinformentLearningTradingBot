...
class TFAgentBase(Agent, ABC):
    ...
    def initialize(self):
        ...
        # if self._checkpoint_filepath and os.path.isdir(self._checkpoint_filepath):
        #     # Check if there are any valid checkpoint files in the specified directory
        #     latest_ckpt = tf.train.latest_checkpoint(self._checkpoint_filepath)
            
        #     if not latest_ckpt:
        #         # If no checkpoint found, this is a fatal error for deployment/evaluation
        #         print(f"FATAL: No valid checkpoint files found in '{self._checkpoint_filepath}'. "
        #               "Agent expected to load trained weights but found none.")
        #         # Raise a specific error to stop execution
        #         raise FileNotFoundError(
        #             f"No valid checkpoint found in {self._checkpoint_filepath}. "
        #             "Cannot initialize agent with trained weights."
        #         )
        #     else:
        #         # If a checkpoint *is* found, proceed to restore the agent's state
        #         print(f"Restoring agent from checkpoint: {latest_ckpt}")
        #         self._checkpointer.initialize_or_restore()
        # else:
        #     # This case means that 'checkpoint_filepath' was either not provided, empty, or not a valid directory
        #     # If it's for inference, this is often a fatal error unless it's a new training run.
        #     print(f"Warning: No valid checkpoint_filepath ('{self._checkpoint_filepath}') or directory provided to TFAgentBase. "
        #           "Agent initializing with random weights (no restore attempted).")
        #     # If this occurs during inference, the RealTimeTrader will catch that self.tradernet_policy is None later.
        # # --- MODIFICATION END ---
